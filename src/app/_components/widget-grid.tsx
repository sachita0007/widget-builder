...
export function WidgetGrid({ campaignId }: { campaignId: string }) {
  const [widgets] = api.widget.getByCampaign.useSuspenseQuery({ campaignId });
    ...
    return (
      <div className="bg-white p-12 rounded-xl shadow-sm border border-gray-100 text-center border-dashed border-2">
        ...
        <p className="text-gray-500 mb-6 max-w-sm mx-auto">
          Create your first widget to start showcasing reviews on your website.
        </p>
        <Link
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      {widgets.map((widget) => (
        <div
          key={widget.id}
          className="group bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-xl hover:shadow-blue-500/5 transition-all duration-300"
        >
          <div className="aspect-[16/10] bg-slate-50 relative flex items-center justify-center border-b border-gray-100 group-hover:bg-blue-50/50 transition-colors overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col items-center gap-4">
              <div className="w-16 h-16 rounded-2xl bg-white shadow-lg flex items-center justify-center transition-transform group-hover:scale-110">
                {widget.template === "GOOGLE" && (
                  <span className="text-2xl">🔍</span>
                )}
                {widget.template === "AGGREGATED" && (
                  <span className="text-2xl">📊</span>
                )}
                {widget.template === "IMAGE" && (
                  <span className="text-2xl">🖼️</span>
                )}
                {widget.template === "AI_GEN" && (
                  <span className="text-2xl">🤖</span>
                )}
              </div>
            ...
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                  Updated{" "}
                  {new Date(widget.updatedAt).toLocaleDateString(undefined, {
                    month: "short",
                    day: "numeric",
                    year: "numeric",
                  })}
              ...
              <button
                ...
                onClick={() => {
                  navigator.clipboard.writeText(
                    `<script src="https://app.freestand.in/widget/${widget.id}.js"></script>`,
                  );
